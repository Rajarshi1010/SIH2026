...
- **Database Engine:** PostgreSQL 16 + TimescaleDB (Time-series hypertables) + PostGIS (Spatial indexing).
- **Spatial Resolution:** Uber H3 Resolution 8 (~500m aperture) and Resolution 9 (~170m).
- **Core Tables in DB:**
  - `known_emitters`: Registered Indian refineries, flare stacks, blast furnaces, and power plants.
  - `thermal_incidents`: Partitioned satellite anomaly records (historical and live).
  - `review_queue`: Human-in-the-Loop review items.
- **Refresh Cadence:** Background worker auto-polls NASA FIRMS every 15 minutes. Data is automatically preserved in TimescaleDB for historical time-lapse playback.
